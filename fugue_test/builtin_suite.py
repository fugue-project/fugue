    ...
    class Tests(TestCase):
        ...
        def test_head_temp(self):
            ...
            with self.dag() as dag:
                ...
                a.partition(by=["a"], presort="b desc").head_temp(n=1).assert_eq(
                    ArrayDataFrame(
            ...
            with self.dag() as dag:
                ...
                a.head_temp(1, presort="a,b desc").assert_eq(
                    ArrayDataFrame([[0, 2]], "a:int,b:int")
                )
            # head_temp presort overrides partition presort
            with self.dag() as dag:
                a = dag.df(
                    [[0, 1], [0, 2], [1, 3], [1, 4], [None, 2], [None, 3]],
                    "a:double,b:double",
                )
                a.partition(by=["a"], presort="b desc").head_temp(
                    n=1, presort="b asc"
                ).assert_eq(
            ...
            with self.dag() as dag:
                ...
                a.head_temp(1, presort="a desc", na_position="first").assert_eq(
                    ArrayDataFrame([[None, 3]], "a:double,b:double")
                )
            # order by with NULL last
            with self.dag() as dag:
                a = dag.df([[0, 1], [1, 2], [None, 3]], "a:double,b:double")
                a.head_temp(1, presort="a desc", na_position="last").assert_eq(
            ...
            with self.dag() as dag:
                with raises(ValueError):
                    a = dag.df([[0, 1], [0, 2], [1, 3]], "a:int,b:int")
                    a.head_temp(0.5).show()
            # na_position not valid
            with self.dag() as dag:
                with raises(ValueError):
                    a = dag.df([[0, 1], [0, 2], [1, 3]], "a:int,b:int")
                    a.head_temp(1, na_position=["True", "False"]).show()
