    ...
    class Tests(TestCase):
        ...
        def test_head(self):
            ...
            with self.dag() as dag:
                ...
                a.partition(by=["a"], presort="b desc").head(n=1).assert_eq(
                    ArrayDataFrame(
            ...
            with self.dag() as dag:
                ...
                a.head(1, presort="a,b desc").assert_eq(
                    ArrayDataFrame([[0, 2]], "a:int,b:int")
                )
            # Head presort overrides partition presort
            with self.dag() as dag:
                a = dag.df(
                    [[0, 1], [0, 2], [1, 3], [1, 4], [None, 2], [None, 3]],
                    "a:double,b:double",
                )
                a.partition(by=["a"], presort="b desc").head(
                    n=1, presort="b asc"
                ).assert_eq(
            ...
            with self.dag() as dag:
                with raises(ValueError):
                    a = dag.df([[0, 1], [0, 2], [1, 3]], "a:int,b:int")
                    a.head(1, na_position=["True", "False"]).show()
