...
def test_parallel():
    ...
    t = timeit(lambda: dag.run(NativeExecutionEngine(
        {"fugue.workflow.concurrency": 10})), number=1)
    assert t < 0.4


def test_parallel_exception():
    dag = FugueWorkflow()
    dag.create(create).process(process).process(
        process, params=dict(sec=0.5)).output(display)
    ...
    t = timeit(lambda: run(dag, NativeExecutionEngine(
        {"fugue.workflow.concurrency": 2})), number=1)  # warmup
    t = timeit(lambda: run(dag, NativeExecutionEngine(
