from fugue.rpc import RPCFuncDict, to_rpc_handler
...
def test_rpc_func_dict():
    ...
    assert to_uuid(RPCFuncDict({})) == to_uuid(to_rpc_handler({}))
    d1 = RPCFuncDict({"f1": f1, "f2": f2, "f3": f3})
    d2 = to_rpc_handler({"f1": f1, "f3": f3, "f2": f2})
    assert 3 == len(d2)
...
def test_determinism():
    ...
    d2 = to_rpc_handler(DeterministicRPCFuncDict({"f3": f3}))
    assert to_uuid(d1) == to_uuid(d2)
