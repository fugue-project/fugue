        ...
        if k in ["partitionby_has", "partitionby_is"]:
            ...
            if k == "partitionby_is":
                assert_or_throw(
                    ...
                    FugueWorkflowValidationError(f"{v} does not match {spec}"),
                )
        if k in ["presort_has", "presort_is"]:
            ...
            if k == "presort_is":
                ...
                assert_or_throw(
                    ...
                    FugueWorkflowValidationError(f"{v} order does not match {spec}"),
                )
...
def validate_input_schema(schema: Schema, rules: Dict[str, Any]) -> None:
    for k, v in rules.items():
        ...
        if k == "input_is":
            assert_or_throw(
                schema == v,
                FugueWorkflowValidationError(f"{v} does not match {schema}"),
            )
